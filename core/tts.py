    ...
    def _ensure_loop(cls) -> asyncio.AbstractEventLoop:
        with cls._loop_lock:
            if cls._bg_loop is None or cls._bg_loop.is_closed():
                ...
                cls._bg_thread = threading.Thread(target=cls._bg_loop.run_forever, daemon=True)
                cls._bg_thread.start()
            return cls._bg_loop

    def synthesize_to_samples(self, text: str) -> tuple[np.ndarray, int]:
        """Synthesize text to float32 numpy samples and sample rate."""
        import miniaudio
        loop = self._ensure_loop()
        try:
            fut = asyncio.run_coroutine_threadsafe(self._synth(text), loop)
            audio_bytes = fut.result(timeout=20)
        except Exception as e:
            print(f"[EdgeTTS] Synth error: {e}")
            audio_bytes = b""
...
class TTSPlayer:
    """
    Wraps any *Engine. Exposes blocking speak() and play_stream() methods
    meant to be called from dedicated background threads.
    """
    ...
    def speak(
        ...
        try:
            ...
            if on_start:
                on_start()
            if hasattr(self._engine, "synthesize_to_samples"):
                samples, rate = self._engine.synthesize_to_samples(text)
                if not self._abort_event.is_set() and samples.size > 0:
                    global TTS_OUTPUT_DEVICE
                    sd.play(samples, rate, device=TTS_OUTPUT_DEVICE)
    ...
    def play_stream(
        self,
        sentence_queue: _queue.Queue[Optional[str]],
        on_start: Optional[Callable] = None,
        on_sentence_start: Optional[Callable[[str], None]] = None,
        on_done: Optional[Callable] = None,
    ) -> None:
        """
        Pipelined sentence-level streaming playback.
        Synthesizes sentence N+1 in a background worker while sentence N is playing.
        sentence_queue receives strings; a sentinel of None indicates completion.
        """
        self._abort_event.clear()
        with self._lock:
            self._playing = True
        ...
        first_started = False
        audio_q: "_queue.Queue[tuple[np.ndarray, int, str] | None]" = _queue.Queue(maxsize=3)

        def _synth_worker():
            try:
                while not self._abort_event.is_set():
                    sent = sentence_queue.get()
                    if sent is None or self._abort_event.is_set():
                        break
                    sent = sent.strip()
                    if not sent:
                        continue
                    try:
                        if hasattr(self._engine, "synthesize_to_samples"):
                            samples, rate = self._engine.synthesize_to_samples(sent)
                        else:
                            samples, rate = np.array([], dtype=np.float32), 24000
                        if not self._abort_event.is_set() and samples.size > 0:
                            audio_q.put((samples, rate, sent))
                    except Exception as exc:
                        print(f"[TTS] Stream synthesis error for '{sent[:30]}': {exc}")
            finally:
                audio_q.put(None)
        ...
        try:
            while not self._abort_event.is_set():
                item = audio_q.get()
                if item is None or self._abort_event.is_set():
                    break
                samples, rate, sent = item
