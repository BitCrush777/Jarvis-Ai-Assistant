...
def _has_gemini_key() -> bool:
    try:
        with open(API_CONFIG_PATH, "r", encoding="utf-8") as f:
            d = json.load(f)
            k = d.get("gemini_api_key", "").strip()
            return bool(k and not k.startswith("nvapi-"))
    except Exception:
        return False
...
def _get_api_key() -> str:
    try:
        with open(API_CONFIG_PATH, "r", encoding="utf-8") as f:
            d = json.load(f)
            k = d.get("gemini_api_key", "").strip()
            return k if (k and not k.startswith("nvapi-")) else ""
    except Exception:
        return ""
...
def _gemini_headlines(n: int = 5) -> tuple[list[str], str]:
    """
    Fetches current headlines via Gemini grounded search, or falls back to DDG news.
    Returns (headline_list, raw_text_for_display).
    """
    if not _gemini_available():
        ddg_results = _ddg_news("top world news today", max_results=n)
        titles = [r["title"] for r in ddg_results if r.get("title")]
        return titles[:n], _format_news("top world news", ddg_results)
