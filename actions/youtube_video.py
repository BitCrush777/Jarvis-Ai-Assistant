...
def _get_api_key() -> str:
    try:
        with open(API_CONFIG_PATH, "r", encoding="utf-8") as f:
            d = json.load(f)
            return d.get("gemini_api_key", "") or d.get("nvidia_api_key", "")
    except Exception:
        return ""
...
def _summarize_video(transcript: str, video_url: str) -> str:
    from core.llm_client import call_llm_text
    max_chars = 40000
    truncated = transcript[:max_chars] + ("..." if len(transcript) > max_chars else "")
    prompt = (
        f"Please summarize this YouTube video transcript clearly and concisely. "
        f"Structure: 1-sentence overview, then 3-5 key points. "
        f"Be direct. Address the user as 'sir'. Match the language of the transcript.\n\n"
        f"TRANSCRIPT:\n{truncated}"
    )
    system = (
    ...
    return call_llm_text(prompt, system=system).strip()


_summarize_with_gemini = _summarize_video
